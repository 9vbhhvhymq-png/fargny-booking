...
var SHAREHOLDERS = [
  { name: "Moritz Fromageot", branch_id: 1 },
  { name: "Sophie Fromageot", branch_id: 1 },
  { name: "Fritz Fromageot", branch_id: 1 },
  { name: "Laurens van der Grinten", branch_id: 2 },
  { name: "Marielle van der Grinten", branch_id: 2 },
  { name: "Jan Henk", branch_id: 1 },
  { name: "Pieter Henk", branch_id: 1 },
  { name: "Anna Daan", branch_id: 2 },
  { name: "Willem Daan", branch_id: 2 },
  { name: "Lotte Remy", branch_id: 3 },
  { name: "Hugo Remy", branch_id: 3 },
  { name: "Clara Frans", branch_id: 4 },
  { name: "Thomas Frans", branch_id: 4 },
  { name: "Eva Marianne", branch_id: 5 },
  { name: "Marc Tom", branch_id: 6 },
  { name: "Julie Tom", branch_id: 6 },
  { name: "Stefan Lino", branch_id: 7 },
  { name: "Rosa Lino", branch_id: 7 },
  { name: "Paul Flop", branch_id: 8 },
  { name: "Marie Bertrand", branch_id: 9 },
];
...
// ═══════════════════════════════════════════════════════
var ADMIN_SEED = { id: 0, name: "Admin", email: "admin@fargny.org", password: "admin", branch_id: 1, is_admin: true };

function loadUsers() {
  try { var d = JSON.parse(localStorage.getItem('fargny_users') || '[]'); return d; } catch(e) { return []; }
}
function saveUsers(users) { localStorage.setItem('fargny_users', JSON.stringify(users)); }
function loadBookings() { try { return JSON.parse(localStorage.getItem('fargny_bookings') || '[]'); } catch(e) { return []; } }
function saveBookings(bk) { localStorage.setItem('fargny_bookings', JSON.stringify(bk)); }
function loadBoardEvents() { try { return JSON.parse(localStorage.getItem('fargny_board_events') || '[]'); } catch(e) { return []; } }
function saveBoardEvents(ev) { localStorage.setItem('fargny_board_events', JSON.stringify(ev)); }

function getRegisteredUsers() {
  var users = loadUsers();
  if (!users.find(function(u) { return u.email === ADMIN_SEED.email; })) {
    users.push(Object.assign({}, ADMIN_SEED));
    saveUsers(users);
  }
  return users;
}
...
function BranchMembersPanel(p){
  var branch=p.branch,onClose=p.onClose,t=p.t,registeredUsers=p.registeredUsers||[];
  var members=registeredUsers.filter(function(u){return u.branch_id===branch.id;});
  return h('div',{style:{position:'fixed',inset:0,background:'rgba(0,0,0,0.3)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center'},onClick:onClose},
    h('div',{onClick:function(e){e.stopPropagation();},style:{background:COLORS.white,borderRadius:16,padding:28,width:360,maxHeight:'70vh',overflow:'auto',boxShadow:'0 12px 48px rgba(0,0,0,0.15)'}},
      ...
      :h('div',{style:{display:'flex',flexDirection:'column',gap:8}},members.map(function(m){
        return h('div',{key:m.id||m.email,style:{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',background:'#F8F5F2',borderRadius:10,borderLeft:'3px solid '+branch.color}},
...
// ═══════════════════════════════════════════════════════
function BookingConfirmModal(p){
  var onConfirm=p.onConfirm,onCancel=p.onCancel,t=p.t,lang=p.lang,weekLabel=p.weekLabel;
  return h('div',{style:{position:'fixed',inset:0,background:'rgba(0,0,0,0.3)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center'},onClick:onCancel},
    h('div',{onClick:function(e){e.stopPropagation();},style:{background:COLORS.white,borderRadius:16,padding:28,width:400,boxShadow:'0 12px 48px rgba(0,0,0,0.15)'}},
      h('h3',{style:{fontSize:18,fontWeight:700,color:COLORS.text,margin:'0 0 16px 0',fontFamily:"'Playfair Display',Georgia,serif"}},t.confirmBooking),
      h('p',{style:{fontSize:14,color:COLORS.text,margin:'0 0 24px 0'}},t.confirmBookQ+' '+weekLabel+'?'),
      h('div',{style:{display:'flex',gap:8}},
        h('button',{onClick:onCancel,style:{flex:1,padding:'11px 0',background:COLORS.bg,border:'none',borderRadius:10,fontSize:14,fontWeight:500,color:'#5A4D40',cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}},t.cancelBtn),
        h('button',{onClick:onConfirm,style:{flex:1,padding:'11px 0',background:COLORS.success,border:'none',borderRadius:10,fontSize:14,fontWeight:600,color:'white',cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}},t.yesBook))));
}
...
function AdminPanel(p){
  var registeredUsers=p.registeredUsers,weeks=p.weeks,year=p.year,bookings=p.bookings,onBooked=p.onBooked,onApproveCancellation=p.onApproveCancellation,onRejectCancellation=p.onRejectCancellation,onUpdatePayment=p.onUpdatePayment,onCreateBoardEvent=p.onCreateBoardEvent,onClose=p.onClose,t=p.t,lang=p.lang;
  ...
  var _evName=useState(''),evName=_evName[0],setEvName=_evName[1];
  var _evDesc=useState(''),evDesc=_evDesc[0],setEvDesc=_evDesc[1];
  var _evStart=useState(''),evStart=_evStart[0],setEvStart=_evStart[1];
  var _evEnd=useState(''),evEnd=_evEnd[0],setEvEnd=_evEnd[1];
  ...
  var handleSubmit=function(){if(!selUser||!selWeek)return;var u=registeredUsers.find(function(x){return x.id===parseInt(selUser)||x.email===selUser;});if(u)onBooked(selWeek,u,selPhase);onClose();};
  var handleCreateEvent=function(){if(!evName||!evStart||!evEnd)return;onCreateBoardEvent({id:Date.now(),name:evName,desc:evDesc,start:evStart,end:evEnd,participants:[]});setEvName('');setEvDesc('');setEvStart('');setEvEnd('');};
  var tabBtn=function(id,label,badge){return h('button',{key:id,onClick:function(){setTab(id);},style:{flex:1,padding:'8px 0',border:'none',borderRadius:8,background:tab===id?COLORS.white:'transparent',color:tab===id?COLORS.text:COLORS.muted,fontWeight:tab===id?600:400,cursor:'pointer',fontSize:10,boxShadow:tab===id?'0 1px 4px rgba(0,0,0,0.08)':'none',fontFamily:"'DM Sans',sans-serif",position:'relative'}},label,badge>0&&h('span',{style:{position:'absolute',top:-4,right:4,background:COLORS.danger,color:'white',fontSize:9,fontWeight:700,padding:'1px 5px',borderRadius:8}},badge));};
  var iS={width:'100%',padding:'10px 14px',border:'1.5px solid #E0D8CF',borderRadius:10,fontSize:14,outline:'none',background:'white',fontFamily:"'DM Sans',sans-serif",marginBottom:10,boxSizing:'border-box'};
  ...
  return h('div',{style:{position:'fixed',inset:0,background:'rgba(0,0,0,0.3)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center'},onClick:onClose},
    h('div',{onClick:function(e){e.stopPropagation();},style:{background:COLORS.white,borderRadius:16,padding:28,width:580,maxHeight:'85vh',overflow:'auto',boxShadow:'0 12px 48px rgba(0,0,0,0.15)'}},
      ...
      h('div',{style:{display:'flex',gap:0,marginBottom:20,background:COLORS.bg,borderRadius:10,padding:3}},
        tabBtn('manual','\u270F\uFE0F '+t.manualTab,0),tabBtn('cancellations','\u274C '+t.cancellationsTab,pending.length),tabBtn('payments','\u{1F4B3} '+t.paymentsTab,0),tabBtn('board','\u{1F451} '+t.boardTab,0)),
      tab==='manual'&&h('div',{style:{display:'flex',flexDirection:'column',gap:14}},
        h('div',null,h('label',{style:{fontSize:12,fontWeight:600,color:'#5A4D40',display:'block',marginBottom:6}},t.owner),
          h('select',{value:selUser,onChange:function(e){setSelUser(e.target.value);},style:iS},h('option',{value:''},t.pickOwner),registeredUsers.map(function(u){var br=FAMILY_BRANCHES.find(function(b){return b.id===u.branch_id;});return h('option',{key:u.email,value:u.email},u.name+' ('+(br?br.name:'?')+')');}))
        ),
        ...
        h('div',null,h('label',{style:{fontSize:12,fontWeight:600,color:'#5A4D40',display:'block',marginBottom:6}},t.week),
          h('select',{value:selWeek,onChange:function(e){setSelWeek(e.target.value);},style:iS},h('option',{value:''},t.pickWeek),weeks.map(function(w){return h('option',{key:w.id,value:w.id},'W'+w.weekNum+': '+formatDate(w.start,lang)+' \u2013 '+formatDate(w.end,lang));}))
        ),
      ...
      tab==='payments'&&h('div',{style:{display:'flex',flexDirection:'column',gap:10}},
        ...
        h('button',{onClick:onClose,style:{marginTop:8,padding:'11px 0',background:'#F5F0EB',border:'none',borderRadius:10,fontSize:14,fontWeight:500,color:'#5A4D40',cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}},t.close)),
      tab==='board'&&h('div',{style:{display:'flex',flexDirection:'column',gap:14}},
        h('h4',{style:{fontSize:14,fontWeight:700,color:COLORS.text,margin:0}},t.createBoardEvent),
        h('input',{style:iS,type:'text',placeholder:t.eventName,value:evName,onChange:function(e){setEvName(e.target.value);}}),
        h('textarea',{style:Object.assign({},iS,{resize:'vertical'}),rows:2,placeholder:t.eventDesc,value:evDesc,onChange:function(e){setEvDesc(e.target.value);}}),
        h('div',{style:{display:'flex',gap:8}},
          h('div',{style:{flex:1}},h('label',{style:{fontSize:11,fontWeight:600,color:COLORS.muted,display:'block',marginBottom:4}},t.eventStart),h('input',{style:iS,type:'date',value:evStart,onChange:function(e){setEvStart(e.target.value);}})),
          h('div',{style:{flex:1}},h('label',{style:{fontSize:11,fontWeight:600,color:COLORS.muted,display:'block',marginBottom:4}},t.eventEnd),h('input',{style:iS,type:'date',value:evEnd,onChange:function(e){setEvEnd(e.target.value);}}))),
        h('button',{onClick:handleCreateEvent,disabled:!evName||!evStart||!evEnd,style:{padding:'11px 0',background:(!evName||!evStart||!evEnd)?'#D4C5B5':COLORS.board,border:'none',borderRadius:10,fontSize:14,fontWeight:600,color:'white',cursor:(!evName||!evStart||!evEnd)?'default':'pointer',fontFamily:"'DM Sans',sans-serif"}},t.createBoardEvent),
        h('button',{onClick:onClose,style:{padding:'11px 0',background:'#F5F0EB',border:'none',borderRadius:10,fontSize:14,fontWeight:500,color:'#5A4D40',cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}},t.close))));
}
...
function WeekRow(p){
  ...
  var barColor=hasMyBooking&&myBooking.open_to_share?COLORS.openToShare:(userBranch?userBranch.color:'#ccc');
  return h('div',{style:{display:'flex',alignItems:'center',padding:'10px 16px',borderRadius:10,background:isPending?'#FEF2F210':hasMyBooking?barColor+'0D':'transparent',transition:'all 0.2s',gap:12,borderLeft:isPending?'3px solid '+COLORS.warning:hasMyBooking?'3px solid '+barColor:'3px solid transparent',opacity:isPending?0.7:1}},
    ...
    h('div',{style:{flex:1,minWidth:0}},
      ...
      hasMyBooking&&!isPending&&h('div',{style:{display:'flex',alignItems:'center',gap:6,marginTop:2}},h('span',{style:{fontSize:12,color:barColor,fontWeight:600}},t.yourBooking+' \u00B7 '+getPhaseMeta(myBooking.phase,t).name+(myBooking.open_to_share?' \u00B7 \u{1F91D}':'')+(myBooking.admin_booked?' \u00B7 '+t.adminBooking:'')),paymentBadge(myBooking.payment_status||'not_paid',t)),
      hasMyBooking&&myBooking.remark&&h('div',{style:{fontSize:11,color:COLORS.muted,marginTop:2,fontStyle:'italic'}},'\u{1F4AC} '+myBooking.remark),
...
// ═══════════════════════════════════════════════════════
// BOOKING DIALOG (with options)
// ═══════════════════════════════════════════════════════
function BookingDialog(p){
  var week=p.week,phase=p.phase,user=p.user,registeredUsers=p.registeredUsers,onConfirm=p.onConfirm,onCancel=p.onCancel,t=p.t,lang=p.lang;
  var _share=useState(false),openToShare=_share[0],setOpenToShare=_share[1];
  var _remark=useState(''),remark=_remark[0],setRemark=_remark[1];
  var _linked=useState([]),linked=_linked[0],setLinked=_linked[1];
  var _useCustom=useState(false),useCustom=_useCustom[0],setUseCustom=_useCustom[1];
  var _checkIn=useState(week?week.start:''),checkIn=_checkIn[0],setCheckIn=_checkIn[1];
  var _checkOut=useState(week?week.end:''),checkOut=_checkOut[0],setCheckOut=_checkOut[1];
  var isRegular=phase==='regular';
  var others=(registeredUsers||[]).filter(function(u){return u.email!==user.email;});
  var toggleLinked=function(email){setLinked(function(prev){return prev.indexOf(email)>=0?prev.filter(function(e){return e!==email;}):prev.concat([email]);});};
  var weekLabel=week?formatDate(week.start,lang)+' \u2014 '+formatDate(week.end,lang):'';
  var customValid=true,nightCount=0;
  ...
  var iS={width:'100%',padding:'10px 14px',border:'1.5px solid '+COLORS.border,borderRadius:10,fontSize:14,outline:'none',fontFamily:"'DM Sans',sans-serif",boxSizing:'border-box'};

  return h('div',{style:{position:'fixed',inset:0,background:'rgba(0,0,0,0.3)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center'},onClick:onCancel},
    h('div',{onClick:function(e){e.stopPropagation();},style:{background:COLORS.white,borderRadius:16,padding:28,width:460,maxHeight:'85vh',overflow:'auto',boxShadow:'0 12px 48px rgba(0,0,0,0.15)'}},
      h('h3',{style:{fontSize:18,fontWeight:700,color:COLORS.text,margin:'0 0 6px 0',fontFamily:"'Playfair Display',Georgia,serif"}},t.confirmBooking),
      h('p',{style:{fontSize:14,color:COLORS.text,margin:'0 0 20px 0'}},t.confirmBookQ+' '+weekLabel+'?'),
      isRegular&&h('div',{style:{marginBottom:16}},
        h('label',{style:{display:'flex',alignItems:'center',gap:8,fontSize:13,color:COLORS.text,cursor:'pointer',marginBottom:10}},
          h('input',{type:'checkbox',checked:useCustom,onChange:function(){setUseCustom(!useCustom);}}),t.customDates),
        useCustom&&h('div',{style:{display:'flex',gap:8}},
          h('div',{style:{flex:1}},h('label',{style:{fontSize:11,fontWeight:600,color:COLORS.muted,display:'block',marginBottom:4}},t.checkIn),h('input',{style:iS,type:'date',value:checkIn,onChange:function(e){setCheckIn(e.target.value);}})),
          h('div',{style:{flex:1}},h('label',{style:{fontSize:11,fontWeight:600,color:COLORS.muted,display:'block',marginBottom:4}},t.checkOut),h('input',{style:iS,type:'date',value:checkOut,onChange:function(e){setCheckOut(e.target.value);}}))),
        useCustom&&!customValid&&h('div',{style:{fontSize:12,color:COLORS.danger,marginTop:6}},t.maxNights)),
      h('label',{style:{display:'flex',alignItems:'center',gap:8,fontSize:13,color:COLORS.text,cursor:'pointer',marginBottom:16}},
        h('input',{type:'checkbox',checked:openToShare,onChange:function(){setOpenToShare(!openToShare);}}),t.openToShare),
      h('div',{style:{marginBottom:16}},
        h('label',{style:{fontSize:12,fontWeight:600,color:COLORS.muted,display:'block',marginBottom:6}},t.addRemark),
        h('textarea',{style:Object.assign({},iS,{resize:'vertical'}),rows:2,value:remark,onChange:function(e){setRemark(e.target.value);}})),
      others.length>0&&h('div',{style:{marginBottom:16}},
        h('label',{style:{fontSize:12,fontWeight:600,color:COLORS.muted,display:'block',marginBottom:6}},t.whoJoining),
        h('div',{style:{maxHeight:120,overflow:'auto',display:'flex',flexDirection:'column',gap:4}},others.map(function(u){
          var sel=linked.indexOf(u.email)>=0;
          return h('label',{key:u.email,style:{display:'flex',alignItems:'center',gap:8,fontSize:13,padding:'6px 10px',borderRadius:8,background:sel?COLORS.success+'10':'#F8F5F2',cursor:'pointer',border:sel?'1px solid '+COLORS.success+'30':'1px solid transparent'}},
            h('input',{type:'checkbox',checked:sel,onChange:function(){toggleLinked(u.email);}}),u.name);}))),
      h('div',{style:{display:'flex',gap:8}},
        h('button',{onClick:onCancel,style:{flex:1,padding:'11px 0',background:COLORS.bg,border:'none',borderRadius:10,fontSize:14,fontWeight:500,color:'#5A4D40',cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}},t.cancelBtn),
        h('button',{onClick:function(){onConfirm({open_to_share:openToShare,remark:remark,linked_members:linked,custom_start:isRegular&&useCustom&&customValid?checkIn:null,custom_end:isRegular&&useCustom&&customValid?checkOut:null});},disabled:isRegular&&useCustom&&!customValid,style:{flex:1,padding:'11px 0',background:(isRegular&&useCustom&&!customValid)?'#D4C5B5':COLORS.success,border:'none',borderRadius:10,fontSize:14,fontWeight:600,color:'white',cursor:(isRegular&&useCustom&&!customValid)?'default':'pointer',fontFamily:"'DM Sans',sans-serif"}},t.yesBook))));
}
...
function BookingView(p){
  ...
  var userBranch=FAMILY_BRANCHES.find(function(b){return b.id===user.branch_id;});
  var branchCounts={};registeredUsers.forEach(function(u){branchCounts[u.branch_id]=(branchCounts[u.branch_id]||0)+1;});
  var branchesWithCount=FAMILY_BRANCHES.map(function(b){return Object.assign({},b,{member_count:branchCounts[b.id]||0});});
  ...
  return h('div',{style:{display:'flex',maxWidth:1200,margin:'0 auto',gap:24,padding:'24px 20px'}},
    ...
    bookDialog&&h(BookingDialog,{week:bookDialog.week,phase:bookDialog.phase,user:user,registeredUsers:registeredUsers,onConfirm:function(opts){onBook(bookDialog.week,bookDialog.phase,opts);setBookDialog(null);},onCancel:function(){setBookDialog(null);},t:t,lang:lang}),
    // SIDEBAR
...
function CalendarView(p){
  var user=p.user,bookings=p.bookings,weeks=p.weeks,year=p.year,phaseConfig=p.phaseConfig,boardEvents=p.boardEvents,registeredUsers=p.registeredUsers,t=p.t,lang=p.lang;
  ...
  var visibleBookings=bookings.filter(function(b){if(b.phase==='regular'||b.phase==='board')return true;if(b.user_id===user.id||b.user_email===user.email)return true;if(user.is_admin)return true;if(b.phase==='clan')return clanRevealed;if(b.phase==='priority')return priorityRevealed;return false;});
  ...
  // Build dayBookings including board events
  var allItems=visibleBookings.map(function(b){var w=weeks.find(function(wk){return wk.id===b.week_id;});return{type:'booking',data:b,start:b.custom_start||(w?w.start:''),end:b.custom_end||(w?w.end:'')};});
  (boardEvents||[]).forEach(function(ev){allItems.push({type:'board',data:ev,start:ev.start,end:ev.end});});

  var dayBookings={};
  for(var dd=1;dd<=daysInMonth;dd++){
    var ds=calYear+'-'+String(calMonth+1).padStart(2,'0')+'-'+String(dd).padStart(2,'0');
    dayBookings[dd]=allItems.filter(function(item){return ds>=item.start&&ds<=item.end;});
  }
  ...
  return h('div',{style:{maxWidth:960,margin:'0 auto',padding:'24px 20px'}},
        ...
        return h('div',{key:rowIdx,style:{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:2}},
          row.map(function(day,ci){
            ...
            return h('div',{key:day,style:{minHeight:64,padding:'3px 2px',position:'relative',border:isToday?'2px solid '+COLORS.accent:'2px solid transparent',borderRadius:6}},
              ...
              bks.slice(0,3).map(function(item,bi){
                var isBoard=item.type==='board';
                var b=item.data;
                var barColor,label,isPending=false;
                if(isBoard){barColor=COLORS.board;label=b.name;}
                else{var br=FAMILY_BRANCHES.find(function(x){return x.id===b.branch_id;});isPending=b.status==='pending_cancellation';barColor=b.open_to_share?COLORS.openToShare:isPending?(br?br.color:'#ccc')+'40':(br?br.color:'#ccc');label=shortName(b.user_name)+(b.open_to_share?' \u{1F91D}':'');}
                var prevDay=ci>0?row[ci-1]:null;var nextDay=ci<6?row[ci+1]:null;
                var itemId=isBoard?'board_'+b.id:b.id;
                var prevHas=prevDay&&(dayBookings[prevDay]||[]).some(function(x){return(x.type==='board'?'board_'+x.data.id:x.data.id)===itemId;});
                var nextHas=nextDay&&(dayBookings[nextDay]||[]).some(function(x){return(x.type==='board'?'board_'+x.data.id:x.data.id)===itemId;});
                var radius=(!prevHas?'4px ':'0 ')+(!nextHas?'4px ':'0 ')+(!nextHas?'4px ':'0 ')+(!prevHas?'4px':'0');
                return h('div',{key:itemId+'_'+bi,onClick:function(e){e.stopPropagation();setPopup(item);},style:{height:16,marginBottom:1,background:barColor+'CC',borderRadius:radius,cursor:'pointer',display:'flex',alignItems:'center',overflow:'hidden',paddingLeft:!prevHas?3:0}},
                  !prevHas&&h('span',{style:{fontSize:8,fontWeight:700,color:'white',whiteSpace:'nowrap',textOverflow:'ellipsis',overflow:'hidden',textDecoration:isPending?'line-through':'none'}},label));
              }));
          }));
      })),
    popup&&h('div',{style:{position:'fixed',inset:0,background:'rgba(0,0,0,0.2)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center'},onClick:function(){setPopup(null);}},
      h('div',{onClick:function(e){e.stopPropagation();},style:{background:COLORS.white,borderRadius:14,padding:24,minWidth:280,maxWidth:400,boxShadow:'0 8px 32px rgba(0,0,0,0.15)'}},
        (function(){
          if(popup.type==='board'){var ev=popup.data;return h(Fragment,null,h('h4',{style:{fontSize:16,fontWeight:700,color:COLORS.board,margin:'0 0 8px 0'}},'\u{1F451} '+ev.name),h('div',{style:{fontSize:13,color:COLORS.text}},formatDate(ev.start,lang)+' \u2013 '+formatDate(ev.end,lang)),ev.desc&&h('div',{style:{fontSize:13,color:COLORS.muted,marginTop:6,fontStyle:'italic'}},ev.desc),h('div',{style:{fontSize:12,color:COLORS.board,marginTop:8}},(ev.participants||[]).length+' '+t.participants),h('button',{onClick:function(){setPopup(null);},style:{marginTop:16,width:'100%',padding:'10px 0',background:COLORS.bg,border:'none',borderRadius:10,fontSize:14,fontWeight:500,color:'#5A4D40',cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}},t.close));}
          var b=popup.data;var br=FAMILY_BRANCHES.find(function(x){return x.id===b.branch_id;});var w=weeks.find(function(wk){return wk.id===b.week_id;});var isPending=b.status==='pending_cancellation';
          var linkedNames=(b.linked_members||[]).map(function(email){var u=(registeredUsers||[]).find(function(x){return x.email===email;});return u?u.name:email;});
          return h(Fragment,null,
            h('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:12}},h('div',{style:{width:12,height:12,borderRadius:3,background:b.open_to_share?COLORS.openToShare:(br?br.color:'#ccc')}}),h('h4',{style:{fontSize:16,fontWeight:700,color:COLORS.text,margin:0,textDecoration:isPending?'line-through':'none'}},b.user_name)),
            h('div',{style:{display:'flex',flexDirection:'column',gap:6,fontSize:13,color:COLORS.text}},
              ...
              w&&h('div',null,h('strong',null,t.week+': '),'W'+w.weekNum+' \u2013 '+formatDate(b.custom_start||w.start,lang)+' \u2013 '+formatDate(b.custom_end||w.end,lang)),
              b.created_at&&h('div',null,h('strong',null,t.booked+': '),formatFullDate(b.created_at,lang)),
              b.remark&&h('div',{style:{fontStyle:'italic',color:COLORS.muted}},h('strong',null,t.remarkLabel+': '),b.remark),
              linkedNames.length>0&&h('div',null,h('strong',null,t.whoJoining+' '),linkedNames.join(', ')),
              b.open_to_share&&h('div',{style:{color:COLORS.openToShare,fontWeight:600}},'\u{1F91D} '+t.openToShare),
              b.open_to_share&&b.user_email&&h('a',{href:'https://wa.me/?text='+encodeURIComponent('Hi '+b.user_name+', can I join your Fargny booking?'),target:'_blank',rel:'noopener',style:{display:'inline-block',padding:'6px 14px',background:'#25D366',color:'white',borderRadius:8,fontSize:12,fontWeight:600,textDecoration:'none',marginTop:4}},'\u{1F4AC} '+t.contactPerson),
...
function StatisticsView(p){
  ...
  var perUser={};confirmed.forEach(function(b){if(!perUser[b.user_name])perUser[b.user_name]={clan:0,priority:0,regular:0,total:0,branch_id:b.branch_id};perUser[b.user_name][b.phase]=(perUser[b.user_name][b.phase]||0)+1;perUser[b.user_name].total++;});
...
function LoginScreen(p){
  ...
  var _selShareholder=useState(''),selShareholder=_selShareholder[0],setSelShareholder=_selShareholder[1];
  ...
  var registeredNames=registeredUsers.map(function(u){return u.name;});
  var availableShareholders=SHAREHOLDERS.filter(function(s){return registeredNames.indexOf(s.name)<0;});
  var selectedSH=SHAREHOLDERS.find(function(s){return s.name===selShareholder;});
  var selectedBranch=selectedSH?FAMILY_BRANCHES.find(function(b){return b.id===selectedSH.branch_id;}):null;

  var handleSubmit=function(e){
    e.preventDefault();setError('');
    if(mode==='login'){
      if(!email.trim()||!pw.trim()) return setError(t.fillAll);
      var found=registeredUsers.find(function(u){return u.email.toLowerCase()===email.trim().toLowerCase()&&u.password===pw;});
      if(!found) return setError(t.invalidLogin);
      onLogin(found);
    } else {
      if(!selShareholder||!email.trim()||!pw.trim()||!pw2.trim()) return setError(t.fillAll);
      if(!email.includes('@')) return setError(t.invalidEmail);
      if(pw!==pw2) return setError(t.passwordMismatch);
      if(registeredUsers.find(function(u){return u.name===selShareholder;})) return setError(t.nameTaken);
      if(registeredUsers.find(function(u){return u.email.toLowerCase()===email.trim().toLowerCase();})) return setError(t.nameTaken);
      var sh=SHAREHOLDERS.find(function(s){return s.name===selShareholder;});
      if(!sh) return setError(t.fillAll);
      var newUser={id:Date.now(),name:sh.name,email:email.trim().toLowerCase(),password:pw,branch_id:sh.branch_id,is_admin:false};
      var users=registeredUsers.concat([newUser]);
      saveUsers(users);
      onLogin(newUser);
    }
  };
    ...
    h('div',{style:{background:COLORS.white,borderRadius:16,padding:'48px 40px',maxWidth:420,width:'100%',boxShadow:'0 4px 24px rgba(44,24,16,0.08)',position:'relative'}},
      ...
      h('form',{onSubmit:handleSubmit},
        ...
        mode==='register'&&h(Fragment,null,
          h('label',{style:{display:'block',fontSize:13,fontWeight:600,color:COLORS.muted,marginBottom:6}},t.pickShareholder),
          h('select',{style:Object.assign({},iS,{cursor:'pointer',background:COLORS.white}),value:selShareholder,onChange:function(e){setSelShareholder(e.target.value);}},
            h('option',{value:''},'-- '+t.pickShareholder+' --'),
            availableShareholders.map(function(s){var br=FAMILY_BRANCHES.find(function(b){return b.id===s.branch_id;});return h('option',{key:s.name,value:s.name},s.name+' ('+br.name+')');})),
          selectedBranch&&h('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:12,padding:'8px 12px',background:selectedBranch.color+'10',borderRadius:8,borderLeft:'3px solid '+selectedBranch.color}},
            h('div',{style:{width:10,height:10,borderRadius:3,background:selectedBranch.color}}),
            h('span',{style:{fontSize:13,fontWeight:600,color:selectedBranch.color}},selectedBranch.name)),
          h('input',{style:iS,type:'email',placeholder:t.email,value:email,onChange:function(e){setEmail(e.target.value);}}),
          h('input',{style:iS,type:'password',placeholder:t.password,value:pw,onChange:function(e){setPw(e.target.value);}}),
          h('input',{style:iS,type:'password',placeholder:t.confirmPassword,value:pw2,onChange:function(e){setPw2(e.target.value);}})),
        ...
        h('p',{style:{fontSize:11,color:'#B0A090',textAlign:'center',marginTop:16}},mode==='login'?'Admin: admin@fargny.org / admin':t.registerFirst))));
}
...
function App(){
  ...
  var _bookings=useState(function(){return loadBookings();}),bookings=_bookings[0],setBookings=_bookings[1];
  ...
  var _boardEvents=useState(function(){return loadBoardEvents();}),boardEvents=_boardEvents[0],setBoardEvents=_boardEvents[1];
  var t=T[lang]||T.en;
  ...
  var updateBookings=function(fn){setBookings(function(prev){var next=fn(prev);saveBookings(next);return next;});};
  ...
  var handleBook=function(week,bookingPhase,opts){
    opts=opts||{};
    var br=FAMILY_BRANCHES.find(function(b){return b.id===user.branch_id;});
    updateBookings(function(prev){return prev.concat([{id:Date.now(),week_id:week.id,year:year,user_id:user.id,user_email:user.email,branch_id:user.branch_id,phase:bookingPhase,admin_booked:false,user_name:user.name,branch_name:br?br.name:'',status:'confirmed',created_at:toISO(new Date()),payment_status:'not_paid',guests:null,open_to_share:opts.open_to_share||false,remark:opts.remark||'',linked_members:opts.linked_members||[],custom_start:opts.custom_start||null,custom_end:opts.custom_end||null}]);});
  ...
  var handleRequestCancel=function(booking){updateBookings(function(prev){return prev.map(function(b){return b.id===booking.id?Object.assign({},b,{status:'pending_cancellation'}):b;});});showToast(t.cancelRequested);};
  var handleApproveCancellation=function(id){updateBookings(function(prev){return prev.filter(function(b){return b.id!==id;});});showToast(t.cancellationApproved);};
  var handleRejectCancellation=function(id){updateBookings(function(prev){return prev.map(function(b){return b.id===id?Object.assign({},b,{status:'confirmed'}):b;});});showToast(t.cancellationRejected);};
  var handleUpdatePayment=function(id,status){updateBookings(function(prev){return prev.map(function(b){return b.id===id?Object.assign({},b,{payment_status:status}):b;});});};
  var handleSaveGuests=function(id,guests){updateBookings(function(prev){return prev.map(function(b){return b.id===id?Object.assign({},b,{guests:guests}):b;});});showToast('\u2705 '+t.saveCosts);};
  var handleAdminBook=function(weekId,targetUser,bookingPhase){
    ...
    updateBookings(function(prev){return prev.concat([{id:Date.now(),week_id:weekId,year:year,user_id:targetUser.id,user_email:targetUser.email,branch_id:targetUser.branch_id,phase:bookingPhase,admin_booked:true,user_name:targetUser.name,branch_name:br?br.name:'',status:'confirmed',created_at:toISO(new Date()),payment_status:'not_paid',guests:null,open_to_share:false,remark:'',linked_members:[],custom_start:null,custom_end:null}]);});
    showToast(t.booked+': '+targetUser.name+' - '+(w?formatDate(w.start,lang):weekId));
  };
  var handleCreateBoardEvent=function(ev){setBoardEvents(function(prev){var next=prev.concat([ev]);saveBoardEvents(next);return next;});showToast('\u{1F451} '+ev.name);};
  var handleSignUpBoard=function(evId){setBoardEvents(function(prev){var next=prev.map(function(ev){if(ev.id===evId&&(ev.participants||[]).indexOf(user.email)<0) return Object.assign({},ev,{participants:(ev.participants||[]).concat([user.email])});return ev;});saveBoardEvents(next);return next;});showToast('\u2705 '+t.signedUp);};
  ...
  return h('div',{style:{minHeight:'100vh',background:COLORS.bg,fontFamily:"'DM Sans','Segoe UI',sans-serif"}},
    selectedBranch&&h(BranchMembersPanel,{branch:selectedBranch,onClose:function(){setSelectedBranch(null);},t:t,registeredUsers:registeredUsers}),
    showAdmin&&h(AdminPanel,{registeredUsers:registeredUsers,weeks:weeks,year:year,bookings:yearBookings,onBooked:handleAdminBook,onApproveCancellation:handleApproveCancellation,onRejectCancellation:handleRejectCancellation,onUpdatePayment:handleUpdatePayment,onCreateBoardEvent:handleCreateBoardEvent,onClose:function(){setShowAdmin(false);},t:t,lang:lang}),
