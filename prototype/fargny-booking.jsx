...
function FeedbackView(p){
  var t=p.t,lang=p.lang,user=p.user,feedbackData=p.feedbackData,onSubmit=p.onSubmit;
  ...
  var radioStyle=function(sel){return{padding:'10px 16px',border:sel?'2px solid '+COLORS.success:'2px solid #E8DFD5',borderRadius:10,background:sel?COLORS.success+'0D':COLORS.white,cursor:'pointer',fontSize:14,fontWeight:sel?600:400,color:sel?COLORS.success:COLORS.text,textAlign:'left',width:'100%',transition:'all 0.15s',fontFamily:"'DM Sans',sans-serif"};};
  var handleExport=function(){
    if(typeof XLSX==='undefined'){alert('SheetJS library not loaded');return;}
    var scaleMap={a:t.fq3a,b:t.fq3b,c:t.fq3c,d:t.fq3d,e:t.fq3e};
    var rows=feedbackData.map(function(f){return{'Date submitted':f.date,'User email':f.email,'Display name':f.name,'Branch':f.branch,'Features requested':f.fq1,'Design changes':f.fq2,'Ease of navigation':scaleMap[f.fq3]||f.fq3,'Other comments':f.fq4};});
    var ws=XLSX.utils.json_to_sheet(rows);var wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Feedback');XLSX.writeFile(wb,'fargny-feedback.xlsx');
  };
  if(submitted) return h('div',{style:{maxWidth:600,margin:'0 auto',padding:'40px 20px',textAlign:'center'}},
    h('div',{style:{background:COLORS.white,borderRadius:16,padding:40,boxShadow:'0 1px 8px rgba(0,0,0,0.04)'}},
      h('div',{style:{fontSize:48,marginBottom:16}},'\u{1F389}'),h('h2',{style:{fontSize:22,fontWeight:700,color:COLORS.text,margin:'0 0 8px 0',fontFamily:"'Playfair Display',Georgia,serif"}},t.feedbackThanks),h('p',{style:{fontSize:14,color:COLORS.muted,margin:'0 0 24px 0'}},t.feedbackSaved),
  ...
  return h('div',{style:{maxWidth:600,margin:'0 auto',padding:'24px 20px'}},
    h('div',{style:{background:COLORS.white,borderRadius:16,padding:32,boxShadow:'0 1px 8px rgba(0,0,0,0.04)'}},
      h('div',{style:{textAlign:'center',marginBottom:28}},h('div',{style:{fontSize:32,marginBottom:8}},'\u{1F4DD}'),h('h2',{style:{fontSize:22,fontWeight:700,color:COLORS.text,margin:'0 0 8px 0',fontFamily:"'Playfair Display',Georgia,serif"}},t.surveyTitle),h('p',{style:{fontSize:14,color:COLORS.muted,margin:0,lineHeight:1.5}},t.surveyIntro)),
      h('div',{style:{display:'flex',flexDirection:'column',gap:28}},
        h('div',null,h('div',{style:qStyle},'1. '+t.fq1),h('textarea',{value:fq1,onChange:function(e){setFq1(e.target.value);},placeholder:t.fq1placeholder,rows:3,style:taStyle})),
        h('div',null,h('div',{style:qStyle},'2. '+t.fq2),h('textarea',{value:fq2,onChange:function(e){setFq2(e.target.value);},placeholder:t.fq2placeholder,rows:3,style:taStyle})),
        h('div',null,h('div',{style:qStyle},'3. '+t.fq3),h('div',{style:{display:'flex',flexDirection:'column',gap:6}},[['a',t.fq3a],['b',t.fq3b],['c',t.fq3c],['d',t.fq3d],['e',t.fq3e]].map(function(pair){return h('button',{key:pair[0],onClick:function(){setFq3(pair[0]);},style:radioStyle(fq3===pair[0])},(fq3===pair[0]?'\u25C9':'\u25CB')+' '+pair[1]);}))),
        h('div',null,h('div',{style:qStyle},'4. '+t.fq4),h('textarea',{value:fq4,onChange:function(e){setFq4(e.target.value);},placeholder:t.fq4placeholder,rows:3,style:taStyle})),
        h('button',{onClick:function(){onSubmit({fq1:fq1,fq2:fq2,fq3:fq3,fq4:fq4});setSubmitted(true);},disabled:!fq3,style:{padding:'14px 0',background:!fq3?'#D4C5B5':COLORS.success,border:'none',borderRadius:12,fontSize:15,fontWeight:600,color:COLORS.white,cursor:!fq3?'default':'pointer',marginTop:8,fontFamily:"'DM Sans',sans-serif"}},t.submitFeedback))),
    h('div',{style:{display:'flex',alignItems:'center',justifyContent:'center',gap:16,marginTop:16}},
      feedbackData.length>0&&h('span',{style:{fontSize:12,color:'#B0A090'}},feedbackData.length+' '+t.feedbackCount),
      user.is_admin&&feedbackData.length>0&&h('button',{onClick:handleExport,style:{padding:'8px 18px',background:'#3B6B9E',border:'none',borderRadius:8,fontSize:13,fontWeight:600,color:'white',cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}},'\u{1F4E5} '+t.downloadFeedback)));
}
...
function App(){
  ...
  var _fd=useState([]),feedbackData=_fd[0],setFeedbackData=_fd[1];
  var t=T[lang]||T.en;
  ...
  var handleFeedbackSubmit=function(data){var br=FAMILY_BRANCHES.find(function(b){return b.id===user.branch_id;});setFeedbackData(function(prev){return prev.concat([Object.assign({},data,{date:new Date().toLocaleString(),email:user.email,name:user.name,branch:br?br.name:'?'})]);});};
  ...
  return h('div',{style:{minHeight:'100vh',background:COLORS.bg,fontFamily:"'DM Sans','Segoe UI',sans-serif"}},
    ...
    view==='feedback'&&h(FeedbackView,{t:t,lang:lang,user:user,feedbackData:feedbackData,onSubmit:handleFeedbackSubmit}),
